...
import { ServiceResponseErrorType } from '../types/ResponseService';
...
function validateName(name: string | undefined): ValidationResponse | null {
  if (!name) {
...
function validatePrice(price: string | undefined): ValidationResponse | null {
  if (!price) {
...
function validateParams({ name, price }: ProductInputtableTypes): ValidationResponse | null {
